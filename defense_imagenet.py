...
parser.add_argument('--num_iter', default=1200, type=int,
...
class Mulnet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(num):
            for j in range(num):
                x_out.append(self.models[i*num + j](x[:, :, h_base*i:h_base*(i+1), w_base*j:w_base*(j+1)]))
        ...
        for i in range(num):
            for j in range(num):
                result[:, :, h_base*i:h_base*(i+1), w_base*j:w_base*(j+1)] += x_out[i*num + j]
        return result
...
def closure():
    ...
    out = net(net_input)    
    total_loss = mse(out.mean(dim=0,keepdim=True), (img_noisy_torch + (noise_target.normal_() * reg_noise_std * 0)) ) 
    total_loss.backward()

    psrn_gt = adapative_psnr(img_noisy_np_, out_cpu.mean(axis=0)) 
